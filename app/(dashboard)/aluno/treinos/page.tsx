...
export default function TreinosPage() {
  ...
  function limparMarcacoesDoTreino(
    treinoId: number
  ) {
    setConcluidos(
      concluidos.filter(
  ...
  async function finalizarTreino(
    ...
    if (!todosFeitos) {
      alert(
        "Finalize todos os exercícios antes de concluir o treino."
      );
    ...
    await supabase
      ...
      .insert({
        ...
        mensagem:
          `Parabéns! Você concluiu o treino "${treino.nome_treino}".`,
      });

    alert(
      "Treino finalizado! Presença registrada."
    );
  ...
  const treinosVisiveis =
    treinos.filter(
      (treino) =>
        !treinosConcluidos.includes(
          treino.id
        )
    );
    ...
    <main className="min-h-screen bg-black text-white p-6 pb-28">
      ...
      <section className="bg-gradient-to-br from-green-500 to-green-700 rounded-[32px] p-6 text-black">
        ...
        <p className="mt-3 font-semibold">
          Marque cada exercício,
          descanse 60s e finalize
          o treino.
        </p>
